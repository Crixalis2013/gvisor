...
func (conn *IPv4Conn) Send(t *testing.T, ipv4 IPv4, additionalLayers ...Layer) {
	t.Helper()

	(*Connection)(conn).send(t, Layers{&ipv4}, additionalLayers...)
}

// Close to clean up any resources held.
func (conn *IPv4Conn) Close(t *testing.T) {
	t.Helper()
...
func (conn *IPv4Conn) ExpectFrame(t *testing.T, frame Layers, timeout time.Duration) (Layers, error) {
	t.Helper()
