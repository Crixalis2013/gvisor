...
func (i *inode) beforeSave() {
	...
	if i.ftype == syscall.S_IFIFO {
		// If this pipe is readable, drain it so that bytes in the pipe can be
		// read after restore. (This is a legacy VFS1 feature.) We don't know
		// if the pipe is readable, so just try reading and tolerate EBADF from
		// the read.
		buf := make([]byte, usermem.PageSize)
		var total int
		for {
			n, err := hostfd.Preadv2(int32(i.hostFD), safemem.BlockSeqOf(safemem.BlockFromSafeSlice(buf[total:])), -1 /* offset */, 0 /* flags */)
			total += int(n)
			if err != nil {
