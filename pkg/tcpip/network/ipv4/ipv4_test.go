	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, f := range test.fragments {
				...
				if firstFragment == nil && ip.FragmentOffset() == 0 {
					firstFragment = hdr.View()
				}
			...
			checker.IPv4(t, replyIPHeader,
				...
				checker.IPFullLength(uint16(header.IPv4MinimumSize+header.ICMPv4MinimumSize+firstFragment.Size())),
				checker.IPv4HeaderLength(header.IPv4MinimumSize),
				checker.ICMPv4(
					checker.ICMPv4Type(header.ICMPv4TimeExceeded),
					checker.ICMPv4Code(header.ICMPv4ReassemblyTimeout),
					checker.ICMPv4Checksum(),
					checker.ICMPv4Payload([]byte(firstFragment)),
				),
			)
