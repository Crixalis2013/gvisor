	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, f := range test.fragments {
				...
				if firstFragment == nil && fragHdr.FragmentOffset() == 0 {
					firstFragment = vv.ToView()
				}
			...
			if firstFragment == nil {
				// We didn't send the first fragment.
				t.Fatalf("unexpected ICMP error message received: %+v", reply)
			}

			vv := buffer.NewVectorisedView(reply.Pkt.Size(), reply.Pkt.Views())
			replyIPHeader := header.IPv6(vv.ToView())
			checker.IPv6(t, replyIPHeader,
				checker.SrcAddr(addr2),
				checker.DstAddr(addr1),
				checker.IPFullLength(uint16(header.IPv6MinimumSize+header.ICMPv6MinimumSize+firstFragment.Size())),
				checker.ICMPv6(
					checker.ICMPv6Type(header.ICMPv6TimeExceeded),
					checker.ICMPv6Code(header.ICMPv6ReassemblyTimeout),
					checker.ICMPv6Payload([]byte(firstFragment)),
				),
			)
		})
	}
